...
import { CheckSquare, Clock, Zap, TrendingUp, Plus, Eye } from "lucide-react";
...
// Utility function for checking today
...
export function Dashboard({ onNavigate }: DashboardProps) {
  ...
  // Default to an empty array if tasks is null/undefined
  const tasks = taskList ?? [];

  const completedToday =
    tasks?.filter(
      (task) => task.status === "completed" && isToday(new Date(task.createdAt))
    ).length || 0;

  const activeTasks =
    tasks?.filter((task) => task.status === "active").length || 0;

  const selfHealedIssues =
    tasks?.filter((task) => task.type === "self-healed").length || 0;

  const successRate =
    tasks && tasks.length > 0
      ? ((completedToday / tasks.length) * 100).toFixed(1) + "%"
      : "0%";
  ...
  const stats = [
    ...
    },
    {
      ...
      value: selfHealedIssues.toString(),
      icon: Zap,
      change: "+3",
            ...
            return (
              ...
              >
                <Card className="glass hover:shadow-xl transition-all duration-300 border-0 overflow-hidden relative">
                  <div
                    className={`absolute top-0 right-0 w-20 h-20 ${stat.gradient} opacity-10 rounded-full -mr-10 -mt-10`}
                  />
                  <CardContent className="p-4 relative">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p
                          className={`text-sm ${stat.color} mt-1 flex items-center gap-1`}
                        >
                      ...
                      <div
                        className={`p-3 rounded-xl ${stat.gradient} shadow-lg`}
                      >
